...
fn kana_row(c: char) -> Option<Row> {
    const A: &[char] = &['あ','か','さ','た','な','は','ま','や','ら','わ','が','ざ','だ','ば','ぱ','ぁ','ゃ'];
    const I: &[char] = &['い','き','し','ち','に','ひ','み','り','ぎ','じ','ぢ','び','ぴ','ぃ'];
    const U: &[char] = &['う','く','す','つ','ぬ','ふ','む','ゆ','る','ぐ','ず','づ','ぶ','ぷ','ゔ','ぅ','ゅ'];
    const E: &[char] = &['え','け','せ','て','ね','へ','め','れ','げ','ぜ','で','べ','ぺ','ぇ'];
    const O: &[char] = &['お','こ','そ','と','の','ほ','も','よ','ろ','を','ご','ぞ','ど','ぼ','ぽ','ぉ','ょ'];
    // Small kana share their row (しょー -> しょう, きれー -> きれい);
    // っ (sokuon) is left unmapped — ー after っ stays unresolved.
